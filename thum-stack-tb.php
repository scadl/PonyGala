<?php
		...
		$finalobj = imagecreatetruecolor(450, 450);
		...
		function ProcessImage(){
			...
			if ( $posx >= 250 ){ $posy+=$stepy; $posx=-55+rand(-20,20); }
			imagecopyresampled($finalobj, $imgobj, $posx+=$stepx, $posy+rand(-20,20), 0, 0, $tbimgwdot, $tbimghgot, $tbimgwd, $tbimghg);
		...
		if ( isset( $_GET['date'] ) ){
			$reqts = $avtd -> query("SELECT file_name FROM arts WHERE category=".$_GET['catn']." AND addate='".$_GET['date']."' ORDER BY random() LIMIT 12");
		} else {			
			$reqts = $avtd -> query("SELECT file_name FROM arts WHERE category=".$_GET['catn']." ORDER BY random() LIMIT 12");
		}
